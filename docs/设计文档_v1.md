...
| 3 | 月度配额重置语义 | ✅ 已实现 | 存储配额（`users.storage_quota`/`used_storage`）持久累计不重置；月度**传输**配额另建 `quota_periods(user_id, period, upload_bytes, download_bytes, upload_quota, download_quota)` 表，每月一行，月初重置。配额配置变更经 SSE 实时推送客户端（见 `docs/protocol-specs/quota.md`）。feat/quota-streaming |
...
| 7 | 下载直传方式 | ✅ 预签名 URL 直连 MinIO | 流式保留作降级；分享下载经能力令牌（capability token）兑换预签名 URL，授权决策（APIServer）与资源访问（TransferServer）解耦 |
| 8 | 分布式追踪 | ✅ 已实现 | OpenTelemetry W3C TraceContext 跨四个边界传播：HTTP（Gin/Hertz 入站）、异步（Redis Streams 事件总线 Event.TraceContext）、客户端中介（ShareDownloadToken.TraceParent）、pub/sub（QuotaChangeEvent.TraceParent）。日志带 trace_id/span_id 关联。导出可配 stdout/otlp/none（见 `docs/protocol-specs/tracing.md`）。feat/observability-tracing |
| 9 | Prometheus 指标 | ✅ 已实现 | 三类指标：RED（中间件自动采集）+ 业务（handler 埋点）+ infra（scrape-time 自定义 Collector）。基数控制（route 用模板非原始 URL、status 归并码类）、跨域 histogram bucket（0.01s..316s）、exemplar 关联（ObserveWithExemplar 从 span context 取 trace_id，Grafana P99→trace 直跳）。Grafana 栈（Prometheus/Loki/Tempo）三支柱联动：metric exemplar→Tempo trace→Loki 日志（见 `docs/protocol-specs/metrics.md`）。feat/observability-metrics |
| 10 | 存储持久层 GC + 事务型 outbox | ✅ 已实现 | 两类结构性问题修复：(a) 哈希引用计数 GC——`DecrRef` 原立即 DELETE 零引用行致 `ListZeroRef` 永远查不到候选，GC 在结构上不可能；改 `zero_ref_at` 墓碑列 + grace window，DecrRef 记墓碑不删行，Upsert 再引用清墓碑，GC 扫墓碑过期候选后 RemoveObject→DeleteIfZeroRef 再校验删除（安全删除序，grace 窗口内并发 re-reference 安全，窗口外残存竞态以长 grace + 监控兜底）。(b) 事件 dual-write——DB 事务 commit 后经 channel→XAdd，commit 与 XAdd 间崩溃事件永久丢失；改 outbox 表与业务同事务写入，OutboxRelay 轮询 `FOR UPDATE SKIP LOCKED`→XAdd→回写 published_at，at-least-once + 消费端 Redis SETNX(eventID) 24h 幂等兜底，`FOR UPDATE SKIP LOCKED` 支持多实例无主并发。repo 方法 `sqlx.ExtContext` 接口化（`*sqlx.DB` 与 `*sqlx.Tx` 都满足）传播事务，消除内联 SQL 漂移（含 `MarkCompleted` size 自赋值 bug）。4 个 cron（TrashPurge/HashGC/SessionExpiry/OutboxRelay）全跑 APIServer 控制面（见 `docs/protocol-specs/persistence-gc.md`）。feat/persistence-gc-outbox |
...
| 2026-08-19 | 决策 #7 更新：MVP 流式 → 预签名 URL 直连 MinIO（流式保留作降级）；分享下载经能力令牌兑换预签名 URL |
| 2026-08-19 | 决策 #10 新增：存储持久层 GC（墓碑 + grace + 安全删除序）+ 事务型 outbox（dual-write 解，at-least-once + 幂等）；repo `sqlx.ExtContext` 接口化传播事务 |
